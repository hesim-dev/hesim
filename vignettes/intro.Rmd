...
2. **Simulation**: The statistical models estimated in Step 1 are combined to construct an economic model. For a given model structure, disease progression, QALYs, and costs are simulated for the target population and treatment strategies of interest.
...
```{r  echo = FALSE, message = FALSE, warning = FALSE}
psm <- c("Independent survival models", "[params_surv_list](../reference/params_surv_list.html)", "[partsurvfit](../reference/partsurvfit.html)")
ictstm <- c("Multi-state model", "[params_surv](../reference/params_surv.html) or [params_surv_list](../reference/params_surv_list.html)", "[flexsurv::flexsurvreg](https://www.rdocumentation.org/packages/flexsurv/versions/1.0.0/topics/flexsurvreg) or [hesim::flexsurvreg_list](../reference/flexsurvreg_list.html)")
tbl <- rbind(psm, ictstm)
colnames(tbl) <- c("Statistical model", "Parameter object", "Model fit object")
knitr::kable(tbl, row.names = FALSE)
```

The easiest way to parameterize a disease model is by fitting a statistical model using `R`. For example, survival models for PSMs and multi-state models for iCTSTMs can be fit using `flexsurvreg()`. A multi-state model can either be fit as a single model with a joint likelihood (a `flexsurvreg` object) or as a list of transition-specific models (a `flexsurvreg_list` object) (see [Section 6 from the flexsurv vignette](https://cran.r-project.org/web/packages/flexsurv/vignettes/flexsurv.pdf) for details). Survival models fit with `flexsurvreg()` for a PSM are stored along with the survival data used for fitting (to facilitate bootstrapping) in a `partsurvfit()` object.
...
colnames(tbl) <- c("Economic model", "Disease progression", "QALYs", "Costs")
knitr::kable(tbl, row.names = FALSE)
```
