...
```{r, warning = FALSE, message = FALSE}
part.surv.medcosts <- form_StateVals(costs.medical.fit, data = statevals.edata, n = n.samples)
```
...
part.surv.drugcosts <- StateVals$new(data = input.dat, params = drugcosts.params)
```
...
```{r, warning = FALSE, message = FALSE}
input.dat <- form_input_data(formula_list(mu = ~ -1 + factor(state_id)), data = statevals.edata)
part.surv.utility <- StateVals$new(data = input.dat, params = utility.params)
```
