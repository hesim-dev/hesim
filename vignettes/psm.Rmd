...
The first step in any health-economic simulation is to estimate the model parameters. Since analyses in `hesim` are, by default, based on sampled values of the parameters from their joint posterior distribution in order to facilitate probabilistic sensitivity analysis (PSA), it is a good idea to determine the number of sampled parameter sets that are desired. 
...
```{r, warning = FALSE, message = FALSE}
drugcosts <- psm4_exdata$costs$drugs$costs
print(drugcosts)
```
...
### Survival models
Survival curves are simulated using `PsmCurves` objects. We can create a `PsmCurves` object using `create_PsmCurves()` as a function of our `partsurvfit` object and a dataset of class `expanded_hesim_data` containing the variables and row identifiers needed for predictive modeling.  
...
Utility and cost values are simulated using models of class `StateVals`. When parameter estimates are based on published sources, it is often easiest to instantiate a `StateVals` object from a `statevals_means` object, which stores the posterior distribution of the mean values assigned to health states. We do this to create a utility model. 

```{r, warning = FALSE, message = FALSE}
utility_means <- stateval_means(values = utility_posterior,
                                strategy_id = strategies_dt$strategy_id,
                                patient_id = patients_dt$patient_id)
utilitymod <- create_StateVals(utility_means)
```

In the utility model, health state values vary by health state. A `stateval_means` object can also be used to store health state values that vary by treatment strategy. We take this approach to create a model for drug costs. 

```{r, warning = FALSE, message = FALSE}
n_strategies <- nrow(strategies_dt) 
n_states <- nrow(states_dt)
drugcost_array <- array(rep(drugcosts, each = n_samples * n_states),
                        dim = c(n_samples, n_states, n_strategies))
