...
pcea.dt <-  pcea(ce, k = seq(0, 300000, 500), sim = "sim", arm = "arm",
...
ggplot(pcea.dt$mce, aes(x = k, y = prob, col = factor(arm))) +
  ...
  scale_x_continuous(breaks = seq(0, 300000, 100000), label = comma) +
  theme(legend.position = "bottom") + scale_colour_discrete(name = "Arm")

## @knitr evpi_plot
evpi.dt <- evpi(ce, e = "qalys", c = "cost", k = seq(0, 300000, 500))
head(evpi.dt)
ggplot(evpi.dt, aes(x = k, y = evpi)) +
  geom_line() + xlab("Willingess to pay") +
  ylab("Expected value of perfect information") +
  scale_x_continuous(breaks = seq(0, 300000, 50000), label = comma) +
  scale_y_continuous(label = scales::dollar) +
  theme(legend.position = "bottom") + scale_colour_discrete(name = "Arm")

## @knitr pcea_pw
pcea.pw.dt <-  pcea_pw(ce,  k = seq(0, 300000, 500), control = "0", e = "qalys", c = "cost",
