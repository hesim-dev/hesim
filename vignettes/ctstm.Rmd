---
title: Continuous time state transition models
date: "`r Sys.Date()`"
output: 
...
In a cost-effectiveness analysis, the treatments strategies of interest, characteristics of the target population, and selected model structure must be specified. We will consider a simple case with two treatment strategies and a heterogeneous population of 1000 patients who differ by age and gender. Furthermore, as discussed above, we will use a model with 3 health states (2 of which are non-death states). 
...
ggamma_fits_cr <- vector(length = n_trans, mode = "list") 
for (i in 1:length(ggamma_fits_cr)){
  ggamma_fits_cr[[i]] <- flexsurv::flexsurvreg(Surv(years, status) ~ factor(strategy_id), 
                                               data = ctstm3_exdata$transitions, 
                                               subset = (trans == i) , 
                                               dist = "weibull") 
}
ggamma_fits_cr <- flexsurvreg_list(ggamma_fits_cr)
```
...
```{r}
ggamma_fits_cf <- vector(length = n_trans, mode = "list") 
for (i in 1:length(ggamma_fits_cf)){
  ggamma_fits_cf[[i]] <- flexsurv::flexsurvreg(Surv(Tstart, Tstop, status) ~ factor(strategy_id), 
                                               data = ctstm3_exdata$transitions, 
                                               subset = (trans == i) , 
                                               dist = "weibull") 
}
ggamma_fits_cf <- flexsurvreg_list(ggamma_fits_cf)
```
...
Drug and medical costs can be specified in a similar fashion. Drug costs are assumed to known with certainty and vary by treatment strategy whereas medical costs are assumed to vary by health state and to follow a gamma distribution. 
...
```{r}
transmod_cr <- create_IndivCtstmTrans(ggamma_fits_cr, transmod_data,
                                      trans_mat = tmat, n = n_samples,
                                      clock = "reset",
                                      start_age = patients$age)
transmod_cf <- create_IndivCtstmTrans(ggamma_fits_cf, transmod_data,
                                      trans_mat = tmat, n = n_samples,
                                      clock = "forward",
                                      start_age = patients$age)
```
...
```{r predicted_hazards_plot}
# Plot hazards
library("ggplot2")
haz <- rbind(predict_haz(ggamma_fits_cr, "reset"),
             predict_haz(ggamma_fits_cf, "forward"))
