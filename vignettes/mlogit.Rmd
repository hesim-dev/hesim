...
## Utility and costs
As in the ["simple Markov cohort"](markov-cohort.html) and ["time inhomogeneous Markov cohort"](markov-inhomogeneous-cohort.html) vignettes on Markov cohort modeling, utility and costs models could be generated using mathematical expressions with `define_model()`. However, in this case it is simpler and more direct (and thus more computationally efficient) to use the `stateval_tbl()` functionality. Separate tables are created for utility, drug costs, and medical costs. 
