...
## Utility and costs
As in the ["getting started"](getting-started.html) and ["time inhomogeneous"](markov-inhomogeneous.html) vignettes on Markov cohort modeling, utility and costs models could be generated using mathematical expressions with `define_model()`. However, in this case it is simpler and more direct (and thus more computationally efficient) to use the `stateval_tbl()` functionality. Separate tables are created for utility, drug costs, and medical costs. 
...
drugcost_tbl <- stateval_tbl(multinom3_exdata$costs$drugs,
                            dist = "fixed",
                            hesim_data = hesim_dat) 
medcost_tbl <- stateval_tbl(ctstm3_exdata$costs$medical,
                            dist = "gamma",
                            hesim_data = hesim_dat)  
```
