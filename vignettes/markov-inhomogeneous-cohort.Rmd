...
```{r}
# ID columns
# One for each treatment strategy patient, time period, AND parameter sample
tpmatrix_id <- function(data, n_samples) {
  id <- data[rep(1:nrow(data), times = n_samples), 
       .(strategy_id, patient_id, grp_id, patient_wt, time_start, time_stop)]
  id[, sample := rep(1:n_samples, each = nrow(data))]
  return(id[, ])
}
...
`tpmatrix()` produces a flattened transition probability matrix for each row of `tpmatrix_id`. We predict the elements of the matrix using the input matrices we derived above. 
...
colnames(tpmat) <- paste0("prob_", colnames(tpmat))
tpmat[1, ]
```
...
```{r}
tparams_transprobs <- tparams_transprobs(cbind(tpmat_id, tpmat))
```
