...
fit_models <- function(clock = c("reset", "forward")) {
  ...
  # Parametric models
  dists <- c(
    "exponential", "weibull", "gompertz", "gamma", "lnorm", "llogis",
    "gengamma"
  )
  ...
  for (i in 1:length(fits)) {
    if (clock == "reset") {
      fits[[i]] <- flexsurvreg(Surv(years, status) ~ trans,
        data = fit_data,
        dist = dists[i]
      )
    } else {
      fits[[i]] <- flexsurvreg(Surv(Tstart, Tstop, status) ~ trans,
        data = fit_data,
        dist = dists[i]
      )
    }
...
sim_stprobs_mstate <- function(fit, n_patients, t_grid,
  ...
  if (clock == "reset") {
    stprobs <- mstate::mssample(
      Haz = cumhaz, trans = tmat, tvec = t_grid,
...
plot_comparison1 <- function(fit, n_patients = 1000, clock) {
  ...
  p <- ggplot(pdat, aes(x = time, y = prob, col = lab)) +
    ...
    facet_wrap(~state) +
    xlab("Years") +
    ylab("Probability in health state") +
...
plot_comparisons(
  fits_reset, 1000, "reset",
  "stprobs-ictstm-reset-hesim-vs-mstate.pdf"
)
plot_comparisons(
  fits_forward, 10000, "forward",
...
# Weibull NMA fit
weiNMA_fit <- flexsurvreg(Surv(years, status) ~ trans,
  data = fit_data,
...
weiNMA_stprobs <- sim_stprobs_fp(
  weiNMA_params, c("a0", "a1"), 10000,
  "Weibull"
)

## Sample and riemann integration
fp_stprobs <- sim_stprobs_fp(
  fp_params, c("gamma0", "gamma1"), 10000,
...
fp_stprobs <- sim_stprobs_fp(
  fp_params, c("gamma0", "gamma1"), 1000,
  "Fractional polynomial"
)
p <- fp_plot(weiNMA_stprobs, fp_stprobs)
...
dat2 <- compute_surv(
  step = step, lower = lower, upper = upper,
  hazfun = fp$hazard
)

## Compare
dat <- rbind(dat1, dat2[, c("time", "surv", "lab")])
ggplot(dat, aes(x = time, y = surv, col = lab)) +
  geom_line()
...
r2 <- replicate(
  1000,
  hesim:::C_test_rsurv(
    time = surv_df$time, cumhaz = surv_df$cumhaz,
    time_inf = FALSE
  )
)
...
comp <- list(
  `Lower = 3` = compare_trandom(lower = 3, n = 100000),
