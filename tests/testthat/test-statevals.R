...
tbl <- data.table(
  strategy_id = strategy_id, grp = grp,
...
test_that(paste0(
  "create_StateVals.stateval_tbl() returns correct output with state_id ",
...
test_that(paste0(
  ...
  mod <- create_StateVals(sv, n = 2, hesim_data = hesim_dat)
  expect_equal(
    nrow(mod$params$value),
    nrow(sv) * n_patients * n_strategies
  )
})

test_that(paste0(
  "create_StateVals.stateval_tbl() works as expected with state_id",
...
test_that(paste0(
  "create_StateVals.stateval_tbl() returns correct output with",
...
test_that("create_StateVals.stateval_tbl() returns correct output when using grp_var", {
  ...
  expect_equal(ncol(mod$params$value), 1)
  expect_equal(
    mod$params$patient_id,
    rep(
      rep(patients$patient_id, each = nrow(states)),
      nrow(strategies)
    )
  )
})
...
test_that(paste0(
  "create_StateVals.stateval_tbl() returns an error if the 'states'",
  ...
  expect_error(
    ...
    paste0(
      "If 'state_id' is not a column in 'object', then ",
...
test_that(paste0(
  "create_StateVals.stateval_tbl() returns an error if the 'patients'",
  "table is not included in hesim_data when it should be"
), {
  sv <- stateval_tbl(
  ...
  expect_error(
    create_StateVals(sv, n = 2),
    paste0(
      "If 'patient_id' is not a column in 'object', then ",
      "'hesim_data' must be included as an argument and ",
...
test_that("stateval_tbl() returns error if the number of rows is wreong", {
  ...
  tbl2 <- rbind(tbl2, data.table(
    strategy_id = 2, state_id = 1, grp = 1,
    mean = .5, se = .1
  ))
  expect_error(
    stateval_tbl(tbl2, dist = "beta"),
    paste0(
      "There must only be one row for each combination ",
...
test_that("stateval_tbl() returns error if the columns aren't correct for the distribution", {
  ...
  # Beta
  expect_error(
    stateval_tbl(tbl[, .(strategy_id, grp, state_id, mean)],
      dist = "beta"
    ),
    paste0(
      "If a beta distribution is specified, then tbl must ",
  ...
  expect_error(
    ...
    paste0(
      "If a gamma distribution is specified, then tbl must ",
  ...
  expect_error(
    ...
    paste0(
      "If a lognormal distribution is specified, then tbl must ",
  ...
  # Uniform
  expect_error(
    stateval_tbl(tbl[, .(strategy_id, grp, state_id, min)],
      dist = "unif"
    ),
    paste0(
      "If a uniform distribution is specified, then tbl must ",
...
test_that("stateval_tbl() returns error if the number of rows is incorrect", {
  expect_error(
    ...
    paste0(
      "The number of rows in 'object' should equal 6 which is the number of",
...
test_that("StateVals$sim", {
  ...
  expect_equal(
    c(costvals_medical$input_data$X$mu %*% t(costvals_medical$params$coefs)),
...
## Cost model
cost_tbl <- stateval_tbl(data.table(
  state_id = 1:3,
...
costmods <- list(create_StateVals(cost_tbl,
  n = n_samples,
...
test_that("sim_costs() throws error with incorrect number of PSA samples", {
  econmod$sim_stateprobs(n_cycles = 5)
  econmod$cost_models[[2]] <- create_StateVals(cost_tbl,
    n = n_samples - 1,
    hesim_data = hesim_dat
  )
  expect_error(
...
test_that("sim_costs() throws error with incorrect number of health states", {
  ...
  econmod$cost_models <- list(create_StateVals(cost_tbl2,
    n = n_samples,
    hesim_data = hesim_dat
  ))
  expect_error(
...
test_that("sim_costs produces correct result with time-varying state values", {
  cost_tbl_tv <- stateval_tbl(
    data.table(
      strategy_id = rep(hesim_dat$strategies$strategy_id, each = 2),
      time_start = c(0, 2, 0, 2),
      est = c(1000, 2, 3000, 10)
    ),
    dist = "fixed"
  )
  econmod$cost_models <- list(create_StateVals(cost_tbl_tv,
    n = n_samples,
    hesim_data = hesim_dat
  ))
  econmod$sim_costs(dr = .03)
...
cost_tbl_starting <- stateval_tbl(
  data.table(
    strategy_id = hesim_dat$strategies$strategy_id,
    est = c(1000, 2000)
  ),
  dist = "fixed"
)
econmod$cost_models <- list(
  create_StateVals(cost_tbl_starting,
    n = n_samples, method = "starting",
    hesim_data = hesim_dat
  )
)
