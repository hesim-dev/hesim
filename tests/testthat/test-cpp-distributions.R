...
test_that("pwexp", {
  ...
  expect_error(
    pwexp$trandom(1, 5),
    paste0(
      "hesim does not currently support sampling from a piecewise ",
...
test_that("survspline", {
  ...
  test_survspline2 <- function(flexsurv_scale, timescale,
    ...
    ### quantile
    expect_equal(flexsurv::qsurvspline(.8,
      gamma = gamma, knots = knots,
      scale = flexsurv_scale, timescale = timescale
    ),
    spline$quantile(.8),
  ...
  test_survspline2(
    flexsurv_scale = "normal", timescale = "identity",
...
test_that("FracPoly", {
  ...
  shape <- 1
  fp <- new(FracPoly,
    gamma = c(log(rate), shape), powers = 1,
    cumhaz_method = "quad", step = -1, random_method = "invcdf"
  )
  expect_equal(
    fp$hazard(5),
