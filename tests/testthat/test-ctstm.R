...
# Costs
## Medical
medcost_tbl <- stateval_tbl(data.frame(
  state_id = states$state_id,
  mean = c(800, 1500),
  se = c(100, 150)
),
...
for (i in 1:length(msfit_list)) {
  msfit_list[[i]] <- flexsurvreg(Surv(years, status) ~ 1,
    data = dat[trans == i],
    dist = "exp"
  )
}
...
for (i in 1:length(msfit_list)) {
  msfit_list_wei[[i]] <- flexsurvreg(Surv(years, status) ~ 1,
    data = dat[trans == i],
    dist = "weibull"
  )
}
...
### Flexsurvreg lists
msfit_list_mix <- flexsurvreg_list(
  msfit_list[[1]], msfit_list[[2]],
  msfit_list_wei[[3]]
)
...
tmat_ebmt4 <- transMat(
  x = list(
    c(2, 3, 5, 6), c(4, 5, 6), c(4, 5, 6), c(5, 6),
    c(), c()
  ),
  names = c("Tx", "Rec", "AE", "Rec+AE", "Rel", "Death")
)
msebmt <- msprep(data = ebmt4, trans = tmat_ebmt4, time = c(
  NA, "rec", "ae",
  "recae", "rel", "srv"
), status = c(
  NA, "rec.s", "ae.s", "recae.s",
  "rel.s", "srv.s"
), keep = c("match", "proph", "year", "agecl"))
covs <- c("match", "proph", "year", "agecl")
msebmt <- expand.covs(msebmt, covs, longnames = FALSE)
msfit <- flexsurvreg(Surv(time / 365.25, status) ~ factor(trans),
  data = msebmt,
  dist = "weibull"
)
...
test_that("create_IndivCtstmTrans has correct fields for transition specific models", {
  m <- create_IndivCtstmTrans(msfit_list,
    input_data = msfit_list_data,
...
test_that("create_IndivCtstmTrans has correct fields for joint model", {
  m <- create_IndivCtstmTrans(msfit,
    input_data = msfit_data,
    trans_mat = tmat_ebmt4,
    clock = "reset", n = n_samples
  )
  expect_equal(m$clock, "reset")
  ...
  m <- create_IndivCtstmTrans(msfit_cf,
    input_data = msfit_data,
    trans_mat = tmat_ebmt4,
    clock = "forward", n = n_samples
  )
...
test_that("IndivCtstmTrans - transition specific", {
  mstate_list <- create_IndivCtstmTrans(msfit_list,
    input_data = msfit_list_data,
    trans_mat = tmat,
    uncertainty = "none"
  )
  ...
  expect_equal(
    hesim_hazard[transition_id == 1][1]$hazard,
    summary(msfit_list[[1]], type = "hazard", t = 3, ci = FALSE)[[1]][1, "est"]
  )
  expect_equal(
    hesim_hazard[transition_id == 2][1]$hazard,
  ...
  expect_equal(
    hesim_cumhazard[transition_id == 1][1]$cumhazard,
    summary(msfit_list[[1]], type = "cumhaz", t = 5)[[1]][1, "est"]
  )
  expect_equal(
    hesim_cumhazard[transition_id == 2][1]$cumhazard,
  ...
  stprobs <- mstate_list$sim_stateprobs(
    t = c(0, 1, 2, 3),
    max_t = 2
  )
  expect_true(inherits(stprobs, "data.table"))
  ...
  # No errors
  expect_error(
    create_IndivCtstmTrans(msfit_list,
      input_data = msfit_list_data,
      trans_mat = tmat,
  ...
  # Errors
  expect_error(create_IndivCtstmTrans(msfit_list,
    input_data = msfit_list_data,
    trans_mat = tmat,
...
test_that("IndivCtstmTrans - joint", {
  mstate <- create_IndivCtstmTrans(msfit,
    input_data = msfit_data,
    trans_mat = tmat_ebmt4,
    uncertainty = "none"
  )
  ...
  expect_equal(
    hesim_hazard[transition_id == 4][1]$hazard,
  ...
  expect_equal(
    hesim_cumhazard[transition_id == 4][1]$cumhazard,
...
test_that("Simulate costs and QALYs", {
  ...
  expect_error(
    ictstm2$sim_qalys(),
    paste0(
      "The number of samples in each 'StateVals' model must equal the ",
  ...
  expect_error(
    ictstm2$sim_qalys(),
    paste0(
      "The number of strategies in each 'StateVals' model must equal the ",
  ...
  expect_error(
    ictstm2$sim_qalys(),
    paste0(
      "The number of patients in each 'StateVals' model must equal the ",
  ...
  expect_error(
    ictstm2$sim_qalys(),
    paste0(
      "The number of states in each 'StateVals' model must equal ",
  ...
  expect_equal(
    ictstm$disprog_[final == 1][sample == 1 & strategy_id == 2 & patient_id == 2, time_stop],
  ...
  ### Simulate
  drugcost_tbl_tv <- stateval_tbl(
    tbl = data.frame(
      strategy_id = rep(strategies$strategy_id, each = 2),
      time_start = c(0, 1.2, 0, 1.2),
      est = c(10000, 500, 12500, 250)
    ),
    dist = "fixed"
  )
  drugcostsmod_tv <- create_StateVals(drugcost_tbl_tv,
    n = n_samples, time_reset = TRUE,
    hesim_data = hesim_dat
  )
  ictstm2$cost_models <- list(
    medical = medcostsmod,
  ...
  test_tv_cost <- function(state) {
    ...
    while (j <= nrow(drug_costs) & (drug_costs[j]$time_start < row$time_elapsed)) {
      time_j <- min(
        row$time_elapsed - drug_costs[j]$time_start,
    ...
    expect_equal(
      ictstm2$costs_[category == "drugs" & sample == row$sample &
  ...
  drugcost_tbl_tv2 <- stateval_tbl(
    tbl = data.frame(
      strategy_id = rep(strategies$strategy_id,
        each = 3
      ),
      time_start = c(
        0, 1.2, 4,
  ...
  drugcostsmod_tv2 <- create_StateVals(drugcost_tbl_tv2,
    n = n_samples,
...
test_that("IndivCtstm - joint", {
  ...
  # Clock-forward
  mstate_cf <- create_IndivCtstmTrans(msfit_cf,
    input_data = msfit_data,
    trans_mat = tmat_ebmt4,
    clock = "forward", n = n_samples
  )
  ...
  mstate_mix <- create_IndivCtstmTrans(params,
    input_data = msfit_data,
    trans_mat = tmat_ebmt4,
...
sim_disprog <- function(params) {
  mstate_fp <- create_IndivCtstmTrans(params,
    input_data = edat,
    trans_mat = tmat_ebmt4
  )
...
test_that("IndivCtstm - fracpoly", {
  # Initial parameters
  coefs_fp <- list(
    gamma0 = matrix(log(1 / 5), nrow = 2, ncol = 1),
