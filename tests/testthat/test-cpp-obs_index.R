...
test_that("obs_index", {
  ...
  expanded_hesim_dat <- expand(hesim_dat, by = c(
    "strategies", "patients",
    "states"
  ))
  ...
  test_obs_index <- function(strategy_idx, patient_idx, state_idx) {
    ...
    C_index <- hesim:::C_test_obs_index(X,
  ...
  X <- input_mats(
    X = list(mu = model.matrix(~age, dat)),
  ...
  test_obs_id <- function(strategy_idx, patient_idx, health_idx, member) {
    ...
    R_id <- X[[member]][obs + 1]
    C_id <- hesim:::C_test_obs_index_ids(X,
      strategy_index = strategy_idx - 1,
      patient_index = patient_idx - 1,
      health_index = health_idx - 1,
      member = member
