...
test_obs_index <- function(strategy_idx, line_idx, patient_idx, state_idx){
  R_index <- which(X$strategy_id == strategy_idx & X$line == line_idx & 
         X$patient_id == patient_idx & X$state_id == state_idx)
...
test_obs_index <- function(strategy_idx, line_idx, patient_idx, transition_idx){
  R_index <- which(X$strategy_id == strategy_idx & X$line == line_idx & 
         X$patient_id == patient_idx & X$transition_id == transition_idx)
  C_index <- hesim:::C_test_obs_index(X, strategy_index = strategy_idx - 1,
                         line_index = line_idx - 1, patient_index = patient_idx - 1,
