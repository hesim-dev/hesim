...
test_that(paste0(
  "as_pfs_os() returns correct output given stable -> progression -> death trajectory",
...
test_that("as_pfs_os() returns error with only one transition", {
  expect_error(
    as_pfs_os(msdata[transition_id == 1],
      patient_vars = "patient_id",
      time_stop = "tstop"
    ),
...
test_that("as_pfs_os() returns error with only two transition that aren't 1 and 2", {
  expect_error(
    as_pfs_os(msdata[transition_id %in% c(1, 3)],
      patient_vars = "patient_id",
      time_stop = "tstop"
    ),
