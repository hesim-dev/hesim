...
test_that("params_surv() works as expected for various distributions", {
  ## exponential
  p <- params_surv(
    coefs = list(matrix(c(1, 2, 3, 4), nrow = 2)),
    dist = "exponential"
  )
...
test_that("params_surv() throws error if hazard scale is wong for spline model", {
  ...
  expect_error(
    ...
    paste0(
      "The auxiliary argument 'scale' must be one of ",
...
test_that("params_surv() throws error if time scale is wong for spline model", {
  expect_error(
    ...
    paste0(
      "The auxiliary argument 'timescale' must be one of ",
...
test_that("params_surv() throws error if numbers of parameters in fractional polynomial model is wrong", {
  expect_error(
    ...
    paste0(
      "The number of parameters in a fractional polynomial model must equal ",
...
test_that("create_params.flexsurv()", {
  ...
  fit <- flexsurv::flexsurvreg(Surv(recyrs, censrec) ~ group,
    data = bc,
  ...
  # spline
  fit <- flexsurv::flexsurvspline(Surv(recyrs, censrec) ~ group,
    data = bc, k = 1,
    scale = "hazard"
  )
  pars_surv <- create_params(fit, n = 2)
