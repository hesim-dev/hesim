...
test_that("mom_beta", {
  ...
  expect_equal(
    beta_mean(beta_params$shape1, beta_params$shape2),
    .8
  )
  expect_equal(
    beta_var(beta_params$shape1, beta_params$shape2),
    .1^2
  )
...
test_that("mom_gamma", {
  ...
  expect_equal(
    gamma_params$shape * gamma_params$scale^2,
    1000^2
  )
  ...
  expect_equal(
    gamma_params$shape / gamma_params$rate,
    10000
  )
  expect_equal(
    gamma_params$shape / gamma_params$rate^2,
    1000^2
  )
})
...
# pdf
expect_equal(
  stats::dweibull(.5, shape, scale),
  hesim::dweibullNMA(.5, a0, a1)
)
expect_equal(
  flexsurv::dweibullPH(.5, shape, scalePH),
  hesim::dweibullNMA(.5, a0, a1)
)

# cdf
expect_equal(
  stats::pweibull(4, shape, scale),
  hesim::pweibullNMA(4, a0, a1)
)

# quantile
expect_equal(
  stats::qweibull(.8, shape, scale),
...
# hazard
expect_equal(
  flexsurv::hweibull(.8, shape, scale),
  hesim::hweibullNMA(.8, a0, a1)
)
expect_equal(
  flexsurv::hweibull(.8, shape, scale, log = TRUE),
...
# rmst
expect_equal(
  flexsurv::rmst_weibull(5, shape, scale),
  hesim::rmst_weibullNMA(5, a0, a1)
)

# mean
expect_equal(
  flexsurv::mean_weibull(shape, scale),
  hesim::mean_weibullNMA(a0, a1)
)

# statistical modeling
## intercept only
s1 <- flexsurvreg(Surv(recyrs, censrec) ~ 1,
  data = bc,
  dist = hesim_survdists$weibullNMA
)
s2 <- flexsurvreg(Surv(recyrs, censrec) ~ 1,
  data = bc,
  dist = "weibullPH"
)
s1_a1 <- s1$res.t["a1", "est"]
s1_a0 <- s1$res.t["a0", "est"]
s2_shape <- s2$res.t["shape", "est"]
s2_scale <- s2$res.t["scale", "est"]
expect_equal(s1_a1, exp(s2_shape) - 1)
expect_equal(s1_a0, log(exp(s2_shape) * exp(s2_scale)))

# covariates on a0
s1 <- flexsurvreg(Surv(recyrs, censrec) ~ group,
  data = bc,
  dist = hesim_survdists$weibullNMA
)
s2 <- flexsurvreg(Surv(recyrs, censrec) ~ group,
  data = bc,
  dist = "weibullPH"
)
s1_a0 <- s1$res.t["a0", "est"]
...
expect_equal(
  log(exp(s2_scale) * exp(s2_shape)),
  s1_a0
)
expect_equal(
  s2_scale + s2_shape, # since shape is a scalar
  s1_a0
)
expect_equal(
  s1$res.t["groupMedium"],
  s2$res.t["groupMedium"]
)

# covariates on a0 and a1
s1 <- suppressWarnings(flexsurvreg(Surv(recyrs, censrec) ~ group,
  data = bc,
  anc = list(a1 = ~group),
  dist = hesim_survdists$weibullNMA
))
s2 <- suppressWarnings(flexsurvreg(Surv(recyrs, censrec) ~ group,
  data = bc,
