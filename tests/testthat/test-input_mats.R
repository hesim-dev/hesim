...
patients <- data.table(
  ...
  ecog = c(
    "Asymptomatic", "Symptomatic (ambulatory)",
    "In bed <50%"
  )
)
states <- data.table(
...
test_that("print.input_mats() works as expected", {
  ...
  expect_output(print(im), paste0(
    "Column binding the ID variables with all ",
...
test_that("create_input_mats.lm() works with times", {
  ...
  expect_true(
    all(c(
      "strategy_id", "patient_id", "state_id", "time_id", "time_start",
      "time_stop", "state_namestate2", "state_namestate3"
    ) %in%
      colnames(imd))
  )
...
test_that(paste0(
  "create_input_mats.flexsurvreg() works with regression ",
  "coefficients on ancillary parameters"
), {
  fit <- flexsurvreg(Surv(recyrs, censrec) ~ group,
    data = bc,
