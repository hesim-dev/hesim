...
formulas <- list(
  "Surv(endpoint1_time, endpoint1_status) ~ age",
...
test_that("create_PsmCurves() produces expected output", {
  psm_curves <- create_PsmCurves(fits_wei,
  ...
  expect_equal(
    as.numeric(psm_curves$input_data$X[[1]]$scale[, "age"]),
    surv_input_data$age
  )
  expect_equal(
    colnames(psm_curves$params$curves1$coefs$shape)[1],
    "(Intercept)"
  )
})
...
test_that("PsmCurves are correct", {
  ...
  ## Splines
  psm_curves <- create_PsmCurves(fits_spline,
    input_data = surv_input_data, n = N,
    uncertainty = "normal"
  )
  ...
  compare_surv_summary <- function(fits, data, fun_name = c(
                                     "survival", "hazard",
                                     "cumhazard", "rmst",
                                     "quantile"
                                   )) {
    fun_name <- match.arg(fun_name)
    psm_curves <- create_PsmCurves(fits,
      input_data = data,
      uncertainty = "none"
    )
    ...
    flexsurv_out <- summary(fits[[1]],
      newdata = data.frame(age = data[1, age]),
  ...
  # Quantiles
  psm_curves <- create_PsmCurves(fits_exp,
    input_data = surv_input_data, n = N,
    uncertainty = "bootstrap", est_data = surv_est_data
  )
...
test_that("sim_stateprobs produces expected results", {
  psm$sim_stateprobs()
  stateprobs_dt <- dt_by_grp(psm$stateprobs_,
    by_var = "state_id",
    value_var = "prob"
  )
