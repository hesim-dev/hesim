...
tprob_array <- array(NA, dim = c(
  n_states, n_states, 2, n_patients,
...
test_that("Extra arguments work with tparams_transprobs.array()", {
  expect_equal(
    tparams_transprobs(tprob_array, times = time_start, grp_id = 1)$grp_id,
    rep(1, prod(dim(tprob_array)[1:4]))
  )
  expect_equal(
    tparams_transprobs(tprob_array, times = time_start, patient_wt = 1)$patient_wt,
    rep(1, prod(dim(tprob_array)[1:4]))
  )
  expect_error(
    tparams_transprobs(tprob_array),
    paste0(
      "'times' cannot be NULL if the number of time intervals ",
      "is greater than 1"
    )
  )
  ...
  paste0(
    "The length of 'grp_id' must be equal to the 3rd dimension of the ",
...
test_that("as.data.table.tparams_transprobs() returns correct output", {
  ...
  expect_equal(
    colnames(x1),
    c(
      "sample", "strategy_id", "patient_id", "time_id", "time_start",
      "time_stop",
...
test_that(paste0(
  "tparams_transprobs() returns the same values with ",
...
test_that("tparams_transprobs.data.table() checks ID attributes", {
  expect_error(
    ...
    paste0(
      "The length of the ID variables is not consistent with the ",
...
test_that("tparams_transprobs() returns the correct class", {
  ...
  expect_true(
    inherits(
      tparams_transprobs(tp, tpmat_id),
      "tparams_transprobs"
    )
  )
})
...
test_that("tparams_transprobs.array() returns an error with the incorrect number of slices", {
  expect_error(
    tparams_transprobs(tparray[, , -1], tpmat_id),
    paste0(
      "The third dimension of the array 'object' must equal ",
...
test_that("CohortDtstmTrans 'trans_mat' must be a matrix of the correct form ", {
  ...
  # Exceptions with $new()
  expect_error(
    CohortDtstmTrans$new(params = tprob, trans_mat = 1),
    msg_matrix
  )
  tmat_bad <- rbind(
    c(0, 0),
...
test_that(paste0(
  "create_CohortDtstmTrans$sim_stateprobs() is consistent with ",
  "predict.multinom()"
), {
  ...
  multinom_probs <- predict(fit_healthy,
    newdata = transmod_data[time_id == 1],
    type = "prob"
  )
...
test_that(paste0("create_CohortDtstmTrans$sim_stateprobs() with multinom() objects"), {
  ...
  stprobs_ref <- sim_markov_chain(
    p = p$p_ref,
    x0 = c(1, 0, 0),
    times = times,
    time_stop = unique(transmod_data$time_stop)
  )
  stprobs_int <- sim_markov_chain(
    p = p$p_int,
    x0 = c(1, 0, 0),
    times = times,
    time_stop = unique(transmod_data$time_stop)
...
test_that(paste0(
  "create_CohortDtstmTrans does not support offset term ",
  ...
  expect_error(
    create_CohortDtstmTrans(transfits2,
      input_data = transmod_data,
      trans_mat = tmat
    ),
...
test_that(paste0(
  "create_CohortDtstmTrans.msm() returns transition probability matrices",
