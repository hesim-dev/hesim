...
sim_onc3_data <- function(n = 2500, seed = NULL) {
  ...
  data <- data.table(
    intercept = 1,
    strategy_id = 1,
    strategy_name = sample(c("SOC", "New 1", "New 2"), n,
      replace = TRUE,
      prob = c(1 / 3, 1 / 3, 1 / 3)
    ),
    patient_id = 1:n,
  ...
  data[, `:=`(
    new1 = ifelse(strategy_name == "New 1", 1, 0),
  ...
  params_wei <- function(shape, mean,
    ...
    scale_coefs <- matrix(c(
      beta_intercept, beta_new1, beta_new2,
      beta_age, beta_female
    ),
    ncol = 5
    )
  ...
  mstate_params <- params_surv_list(
    ...
    # 3. P -> D
    params_wei(
      shape = 3.5, mean = 8, beta_new1 = log(1),
