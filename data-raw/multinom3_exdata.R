...
sim_mlogit <- function(n = 10000) {
  ...
  predict_probs <- function(x, beta, state) {
    ...
    if (sum(state == "Healthy") > 0) {
      ...
      z[which(state == "Healthy"), ] <- cbind(
        1, exp(x_healthy %*% beta$h$s),
  ...
  for (t in 1:n_years) {
    ...
    year_dummies <- matrix(year_dummies,
      ncol = 2, nrow = length(alive),
      byrow = TRUE
    )
