...
/*******************************************************************************
* This is an adaptation of the integrate.c function from the R Stats Package.
* All function pointers have been replaced with functors. 
*******************************************************************************/
...
template <typename Func>
inline void rdqk15i(Func f,
		    double *, int *, double * , double *,
		    double *, double *, double *, double *);

template <typename Func>
inline void rdqagse(Func f, double *, double *,
		    double *, double *, int *, double *, double *,
		    int *, int *, double *, double *, double *,
		    double *, int *, int *);

template <typename Func>
inline void rdqk21(Func f,
		   double *, double *, double *, double *, double *, double *);

inline void rdqpsrt(int *, int *, int *, double *, double *, int *, int *);

inline void rdqelg(int *, double *, double *, double *, double *, int *);
...
template <typename Func>
void Rdqagi(Func f, double *bound, int *inf,
...
***end prologue  dqagi */
    ...
    *abserr = 0.;
    ...
    rdqagie(f, bound, inf, epsabs, epsrel, limit, result, abserr, neval, ier,
...
template <typename Func>
inline void rdqagie(Func f, double *bound, int *inf, double *
...
/*         determine the interval to be mapped onto (0,1).
    ...
    rdqk15i(f, &boun, inf, &c_b6, &c_b7, result, abserr, &defabs, &resabs);
...
/*           test on accuracy */
    ...
    *last = 1;
    ...
    iord[1] = 1;
    dres = fabs(*result);
    errbnd = R::fmax2(*epsabs, *epsrel * dres);
...
/*           bisect the subinterval with nrmax-th largest error estimate. */
	...
	b2 = blist[maxerr];
	erlast = errmax;
	rdqk15i(f, &boun, inf, &a1, &b1, &area1, &error1, &resabs, &defab1);
	rdqk15i(f, &boun, inf, &a2, &b2, &area2, &error2, &resabs, &defab2);
...
/*           improve previous approximations to integral
	...
	rlist[maxerr] = area1;
	rlist[*last] = area2;
	errbnd = R::fmax2(*epsabs, *epsrel * fabs(area));
...
/*           perform extrapolation.  L60: */
	...
	*result = reseps;
	correc = erlarg;
	ertest = R::fmax2(*epsabs, *epsrel * fabs(reseps));
	if (*abserr <= ertest) {
	    break;
	}
...
/*           test on divergence */
L110:
    if (ksgn == -1 && R::fmax2(fabs(*result), fabs(area)) <= defabs * .01) {
	goto L130;
    }
...
template <typename Func>
void Rdqags(Func f, double *a, double *b,
	    double *epsabs, double *epsrel,
	    double *result, double *abserr, int *neval, int *ier,
...
/*         prepare call for dqagse. */
    ...
    rdqagse(f, a, b, epsabs, epsrel, limit, result, abserr, neval, ier,
	    work, &work[l1], &work[l2], &work[l3], iwork, last);
...
template <typename Func>
inline void rdqagse(Func f, double *a, double *b, double *
	     epsabs, double *epsrel, int *limit, double *result,
	     double *abserr, int *neval, int *ier, double *alist,
	     double *blist, double *rlist, double *elist, int *
	     iord, int *last)
...
/*           ----------------------------------- */
    ...
    oflow = DBL_MAX;
    ierro = 0;
    rdqk21(f, a, b, result, abserr, &defabs, &resabs);
...
/*           bisect the subinterval with the nrmax-th largest error estimate. */
	...
	b2 = blist[maxerr];
	erlast = errmax;
	rdqk21(f, &a1, &b1, &area1, &error1, &resabs, &defab1);
	rdqk21(f, &a2, &b2, &area2, &error2, &resabs, &defab2);
...
/* L5: */
    ...
    f(vec, 15); /* -> new vec[] overwriting old vec[] */
    if (*inf == 2) f(vec2, 15);
    fval1 = vec[0];
...
/* L20: */
	...
	*abserr = *resasc * R::fmin2(1., pow(*abserr * 200. / *resasc, 1.5));
    }
    if (*resabs > uflow / (epmach * 50.)) {
	*abserr = R::fmax2(epmach * 50. * *resabs, *abserr);
    }
    return;
} /* rdqk15i_ */

inline void rdqelg(int *n, double *epstab, double *
...
template <typename Func>
inline void  rdqk21(Func f, double *a, double *b, double *result,
		    double *abserr, double *resabs, double *resasc)
{
    /* Initialized data */
...
inline void rdqpsrt(int *limit, int *last, int *maxerr,
...
double quad(Func f, double lower, double upper, double &abserr,
  ...
  vectorize<Func> vecf(f);
  Rdqags(vecf, &lower, &upper, &epsabs, &epsrel, &result,
