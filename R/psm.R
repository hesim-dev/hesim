...
PsmCurves <- R6::R6Class(
  "PsmCurves",
  private = list(
    summary = function(x, type = c(
                         "hazard", "cumhazard", "survival",
                         "rmst", "quantile"
                       ),
                       dr = 0) {
      self$check()
...
create_PsmCurves.flexsurvreg_list <- function(object, input_data, n = 1000,
  ...
  uncertainty <- deprecate_bootstrap(
    list(...)$bootstrap, uncertainty,
    is_uncertainty_missing
  )
  ...
  public = list(
    ...
    sim_qalys = function(dr = .03, integrate_method = c("trapz", "riemann_left", "riemann_right"),
      ...
      self$qalys_ <- sim_qalys(
        self$stateprobs_, self$utility_model, dr,
