...
#'
#' @description
#' Contains methods that can be used to summarize both individual- and
#' cohort-level continuous time state transition models. That is, this class is
#' relevant for both Markov and semi-Markov multi-state models and does not
#' depend on the methodology used for prediction of state probabilities.
  ...
  private = list(
    check_base = function() {
      if (!inherits(self$input_data, "input_mats")) {
        stop(
          "'input_data' must be an object of class 'input_mats'",
          call. = FALSE
        )
      }
      if (!inherits(self$params, c(
        "params_surv",
        "params_surv_list"
      ))) {
        stop(
          "Class of 'params' is not supported. See documentation.",
          call. = FALSE
        )
      }
      ...
      if (inherits(self$params, "params_surv_list")) {
        if (max(self$trans_mat, na.rm = TRUE) != length(self$params)) {
          stop(paste0(
            "The number of models in 'params' must equal the maximum integer ",
            "in 'trans_mat'."
          ),
          call. = FALSE
          )
...
indiv_ctstm_sim_disease <- function(trans_model, max_t = 100, max_age = 100,
  ...
  disprog <- C_ctstm_sim_disease(
    trans_model, trans_model$start_state - 1,
...
indiv_ctstm_sim_stateprobs <- function(
    disprog = NULL, trans_model = NULL, t, ...
  ) {
  ...
  ## Computation
  stprobs <- C_ctstm_indiv_stateprobs(
    disprog,
    t,
    n_samples,
    n_strategies,
...
#'
#' @description
#' Simulate health state transitions in an individual-level continuous time
#' state transition model using parameters from a multi-state model.
#' @format An [R6::R6Class] object.
...
IndivCtstmTrans <- R6::R6Class(
  ...
  private = list(
    check_history = function(field) {
      ...
      if (length(field) != 1 & length(field) != self$input_data$n_patients) {
        stop(paste0(
          "The length of '", field_name, "' must either be 1 or the number ",
  ...
  public = list(
    ...
    #' by sample from the probabilistic sensitivity analysis (PSA), treatment
    #' strategy and patient. Must be an object of class [`input_mats`]. If
    #' `params` contains parameters from a list of models (i.e., of class
    #' [`params_surv_list`]), then `input_data` must contain a unique row for
    #' each treatment strategy and patient; if `params` contains parameters from
    #' a joint model (i.e., of class [`params_surv`]), then `input_data` must
    #' contain a unique row for each treatment strategy, patient, and
    #' transition.
    input_data = NULL,

    #' @field trans_mat A transition matrix describing the states and
    #' transitions in a multi-state model in the format from the
    #' [`mstate`][mstate::mstate] package. See the documentation for the
    #' argument `"trans"` in [`mstate::msprep`].
    trans_mat = NULL,

    #' @field start_state A scalar or vector denoting the starting health state.
    #' Default is the first health state. If a vector, must be equal to the
    #' number of simulated patients.
    start_state = NULL,

    #' @field start_age A scalar or vector denoting the starting age of each
    #' patient in the simulation. Default is 38. If a vector, must be equal to
    #' the number of simulated patients.
    start_age = NULL,

    #' @field death_state The death state in `trans_mat`. Used with `max_age`
    #' in `sim_disease` as patients transition to this state upon reaching
    #' maximum age. By default, it is set to the final absorbing state (i.e., a
    #' row in `trans_mat` with all NAs).
    death_state = NULL,

    #' @field clock "reset" for a clock-reset model, "forward" for a
    #' clock-forward model, and "mix" for a mixture of clock-reset and
    #' clock-forward models. A clock-reset model is a semi-Markov model in which
    #' transition rates depend on time since entering a state. A clock-forward
    #' model is a Markov model in which transition rates depend on time since
    #' entering the initial state. If `"mix"` is used, then `reset_states` must
    #' be specified.
    clock = NULL,
    ...
    initialize = function(
      ...
      if (!is.null(death_state)) {
        if (death_state > nrow(trans_mat)) {
          stop(
            "'death_state' cannot be larger than the number of rows in ",
            "'trans_mat'.",
            call. = FALSE
          )
        } else {
    ...
    #' @param max_t A scalar or vector denoting the length of time to simulate
    #' the model. If a vector, must be equal to the number of simulated
    #' patients.
    #' @param max_age A scalar or vector denoting the maximum age to simulate
    #' each patient until. If a vector, must be equal to the number of simulated
    #' patients.
    #' @param progress An integer, specifying the PSA iteration (i.e., sample)
    #' that should be printed every progress PSA iterations. For example, if
    #' progress = 2, then every second PSA iteration is printed. Default is
    #' NULL, in which case no output is printed.
    ...
    #' `NULL`, then this will be simulated prior to computing state
    #' probabilities using `IndivCtstm$sim_disease()`.
...
#' [`expand.hesim_data`].
#' @param trans_mat The transition matrix describing the states and transitions
#' in a multi-state model in the format from the [`mstate`][mstate::mstate]
#' package. See [`IndivCtstmTrans`].
#' @param clock "reset" for a clock-reset model, "forward" for a clock-forward
#' model, and "mix" for a mixture of clock-reset and clock-forward models. See
#' the field `clock` in [`IndivCtstmTrans`].
#' @param reset_states A vector denoting the states in which time resets. See
#' the field `reset_states` in [`IndivCtstmTrans`].
#' @inheritParams create_CohortDtstmTrans
#' @param ... Further arguments passed to `IndivCtstmTrans$new()` in
#' [`IndivCtstmTrans`].
...
create_IndivCtstmTrans_flexsurvreg <- function(
    object,
    input_data,
    trans_mat,
  ...
  do.call(
    ...
    c(
      list(
        input_data = input_mats, params = params, trans_mat = trans_mat,
...
#' @export
#' @rdname create_IndivCtstmTrans
create_IndivCtstmTrans.flexsurvreg_list <- function(
    object,
    input_data,
    trans_mat,
    clock = c("reset", "forward"),
...
#' @export
#' @rdname create_IndivCtstmTrans
create_IndivCtstmTrans.flexsurvreg <- function(
    object,
    input_data,
    trans_mat,
    clock = c("reset", "forward"),
...
#' @export
#' @rdname create_IndivCtstmTrans
create_IndivCtstmTrans.params_surv <- function(
    object,
    input_data,
    trans_mat,
    clock = c("reset", "forward", "mix"),
  ...
  input_mats <- create_input_mats(object, input_data)
  return(IndivCtstmTrans$new(
    input_data = input_mats, params = object, trans_mat = trans_mat,
...
#' @export
#' @rdname create_IndivCtstmTrans
create_IndivCtstmTrans.params_surv_list <- function(
    object,
    input_data,
    trans_mat,
    clock = c("reset", "forward", "mix"),
...
#' documentation describes the class for the cost and utility models. An
#' [`IndivCtstmTrans`] object is typically created using
...
#' available by fitting a multi-state models.
#' `vignette("markov-inhomogeneous-indiv")` shows how the time inhomogeneous
#' Markov cohort model in `vignette("markov-inhomogeneous-cohort")` can be
#' developed as an individual patient simulation; in doing so, it shows how
#' `IndivCtstm` models can be used even without access to patient-level data.
#' @references [Incerti and Jansen (2021)](https://arxiv.org/abs/2102.09437).
#' See Section 2.2 for a mathematical description of an individual-level CTSTM
#' and Section 4.1 for an example in oncology.
...
#' @param by_patient If `TRUE`, then QALYs and/or costs are computed at the
#' patient level. If `FALSE`, then they are averaged across patients by health
#' state.
#' @export
IndivCtstm <- R6::R6Class("IndivCtstm",
  private = list(
    ...
    sim_ev = function(stateval_list, dr, stateval_type = c("costs", "qalys"),
      ...
      if (is.null(self$disprog_)) {
        stop(
          "You must first simulate disease progression using '$sim_disease'.",
          call. = FALSE
        )
      }
      check_dr(dr)
      check_state_vals(stateval_list, self$disprog_, object_name = "disprog")
      ...
      if (is.null(private$disprog_idx)) {
        ...
        private$disprog_idx <- self$disprog_[
          , c("strategy_idx", "patient_idx"), with = FALSE
        ]
      ...
      if (!(length(max_t) %in% c(1, n_cats))) {
        stop(
          "'max_t' must either equal the number of cost categories or be of ",
          "length 1.",
          call. = FALSE
        )
      }
      ...
      for (i in 1:n_cats) {
        for (j in 1:n_dr) {
          if (stateval_list[[i]]$method == "wlos") {
            C_ev <- C_indiv_ctstm_wlos(
              # Note: C++ re-indexing done at C level for disprog_
              self$disprog_,
            ...
            stop(
              "The 'StateVals' 'method' must either be 'wlos' or 'starting'."
            )
          }
          self$disprog_[, ev := C_ev]
          if (lys) {
            C_los <- C_indiv_ctstm_los(
              # Note: C++ re-indexing done at C level for disprog_
              self$disprog_,
              private$disprog_idx$strategy_idx,
              private$disprog_idx$patient_idx,
          ...
          if (by_patient == TRUE) {
            by_cols <- c(
              "sample", "strategy_id", "patient_id", "grp_id", "from"
            )
            ...
            ev_list[[counter]] <- ev_list[[counter]][
              CJ(sample, strategy_id, patient_id, grp_id, from,
              unique = TRUE
            )]
          } else {
            ...
            setkeyv(ev_list[[counter]], by_cols)
            ev_list[[counter]] <- ev_list[[counter]][
              CJ(sample, strategy_id, grp_id, from,
              unique = TRUE
            )]
          }
          ...
          # Replace padded NA's with 0's
          ev_list[[counter]][, ev := ifelse(is.na(ev), 0, ev)]
          if (lys) {
            self$disprog_[, "lys" := NULL]
            # Replace padded NA's with 0's
            ev_list[[counter]][, lys := ifelse(is.na(lys), 0, lys)]
          }
          counter <- counter + 1
        }
      }
  ...
  public = list(
    #' @field trans_model The model for health state transitions.
    #' Must be an object of class [`IndivCtstmTrans`].
    trans_model = NULL,
    ...
    #' @description
    #' Simulate disease progression (i.e., individual trajectories through a
    #' multi-state model) using `IndivCtstmTrans$sim_disease()`.
    #' @param max_t  A scalar or vector denoting the length of time to simulate
    #' the model. If a vector, must be equal to the number of simulated
    #' patients.
    #' @param max_age A scalar or vector denoting the maximum age to simulate
    #' each patient until. If a vector, must be equal to the number of
    #' simulated patients.
    #' @param progress An integer, specifying the PSA iteration (i.e., sample)
    #' that should be printed every `progress` PSA iterations. For example, if
    #' `progress = 2`, then every second PSA iteration is printed. Default is
    ...
    sim_disease = function(max_t = 100, max_age = 100, progress = NULL) {
      ...
      self$disprog_ <- self$trans_model$sim_disease(
        max_t = max_t,
        max_age = max_age,
        progress = progress
      )
    ...
    #' @param t A numeric vector of times.
    #' @return An instance of `self` with simulated output of class
    #' [`stateprobs`] stored in `stateprobs_`.
    ...
    sim_qalys = function(dr = .03, type = c("predict", "random"),
      ...
      type <- match.arg(type)
      self$qalys_ <- private$sim_ev(
        list(self$utility_model),
        dr,
        "qalys",
        type,
        by_patient,
    ...
    #'   so that costs are computed over the entire duration that a patient is
    #'   in a given health state. If time varies by each cost category, then
    #'   time can also be passed as a numeric vector of length equal to the
    #'   number of cost categories (e.g., `c(1, 2, Inf, 3)` for a model with
    #'   four cost categories).
    #' @return An instance of `self` with simulated output of class [costs]
    #' stored in `costs_`.
    sim_costs = function(
      dr = .03, type = c("predict", "random"), by_patient = FALSE, max_t = Inf
    ) {
      if (!is.list(self$cost_models)) {
        ...
        for (i in seq_along(self$cost_models)) {
