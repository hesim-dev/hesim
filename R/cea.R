...
cea_pw.default <- function(x, k = seq(0, 200000, 500), comparator,
  ...
  # estimates
  outcomes <- c(e, c)
  delta <- calc_incr_effect(
    sim_treat, sim_comparator, sample, strategy, grp, outcomes,
    n_samples, n_strategies, n_grps
  )
  setnames(delta, paste0("i", e), "ie")
  setnames(delta, paste0("i", c), "ic")
  ceac <- ceac(delta, k, strategy, grp,
    e = "ie", c = "ic",
    n_samples, n_strategies, n_grps
  )
...
cea_pw.ce <- function(
  ...
  res <- cea_pw(sim,
    k = k, comparator = comparator, sample = "sample",
    strategy = "strategy_id", grp = "grp_id",
    e = "qalys", c = "costs"
  )
  return(res)
...
nmb_summary <- function(x, k, strategy, grp, e, c) {
  ...
  nmb_dt <- data.table(
    strategy = rep(x[[strategy]], times = length(k)),
...
icer <- function(x, prob = .95, k = 50000, labels = NULL, ...) {
  ...
  delta[, inmb := k * ie - ic]
  inmb <- delta[, list(
    inmb_lower = stats::quantile(inmb, alpha$lower),
    inmb_mean = mean(inmb),
  ...
  tbl <- melt(tbl,
    id.vars = c(strategy, grp),
    measure.vars = list(
      c("ie_mean", "ic_mean", "inmb_mean", "icer"),
...
format.icer <- function(x, digits_qalys = 2, digits_costs = 0,
  ...
  y[, value := fcase(
    ...
    format_ci(estimate, lower, upper,
      costs = TRUE,
      digits = digits_costs
    ),
