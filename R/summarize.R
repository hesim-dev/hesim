...
incr_effect <- function(x, comparator, sample, strategy, grp = NULL, outcomes) {
  ...
  # estimation
  return(calc_incr_effect(
    x_treat, x_comparator, sample, strategy, grp, outcomes,
...
#' surv <- data.table(
#'   strategy = rep(strategies, each = length(t)),
#'   t = rep(t, 2),
#'   surv = c(surv1, surv2)
#' )
#' surv_quantile(surv,
