...
#'
#' @export
stateval_tbl <- function(tbl, dist = c(
                           "norm", "beta", "gamma",
  ...
  if (dist == "norm") {
    if (!all(c("mean", "sd") %in% cols)) {
      msg <- stop(
        "If a normal distribution is specified, then tbl must ",
    ...
    if (!all(c("mean", "se") %in% cols) &
      ...
      msg <- stop(
        "If a beta distribution is specified, then tbl must either ",
    ...
    if (!all(c("mean", "se") %in% cols) &
      ...
      msg <- stop(
        "If a gamma distribution is specified, then tbl must either ",
  ...
  if (length(id_vars) == 1) {
    ...
    id_vars_msg <- paste0(
      paste(id_vars[1:length(id_vars) - 1], collapse = ", "),
      ", and ", id_vars[length(id_vars)]
    )
  }
  if (!all(tbl2[, .N, by = id_vars]$N == 1)) {
    stop(paste0(
      "There must only be one row for each combination of ",
      id_vars_msg,
      " in 'tbl'."
    ),
    call. = FALSE
  ...
  if (nrow(tbl2) != expected_n) {
    stop(paste0(
      "The number of rows in 'object' should equal ", expected_n,
  ...
  # Nice sorting
  id_cols <- c(
    "sample", "strategy_id", "patient_id", grp_var, "state_id", "time_id",
...
create_StateVals.stateval_tbl <- function(object, hesim_data = NULL, n = 1000, ...) {
  ...
  need_hesim_data <- function(var) {
    if (is.null(object[[var]])) {
      ...
      if (is.null(hesim_data[[name]])) {
        msg <- paste0(
          "If '", var, "' is not a column in 'object', ",
  ...
  if (is.null(object[["patient_id"]])) {
    if (is.null(hesim_data[["patients"]][["patient_id"]]) &
      ...
      stop(paste0(
        "If 'patient_id' is not included as a column in `object`, ",
  ...
  if (attr(object, "dist") == "norm") {
    ...
    if (all(c("shape1", "shape2") %in% colnames(tbl))) {
      ...
      mu <- mom_fun_rng(n,
        rng_fun = "rbeta", mom_fun = "mom_beta",
    ...
    if (all(c("shape", "rate") %in% colnames(tbl))) {
      ...
      mu <- mom_fun_rng(n,
        rng_fun = "rgamma", mom_fun = "mom_gamma",
        mean = tbl$mean, sd = tbl$se
      )
    }
...
create_StateVals.eval_model <- function(object, cost = TRUE, name = NULL,
  ...
  return(do.call("create_StateVals",
    args = c(
      list(
        object = stateval_tbl(out_dt, dist = "custom"),
        n = object$n
      ),
      init_args
    )
