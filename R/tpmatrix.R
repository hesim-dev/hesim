...
tpmatrix <- function(..., complement = NULL, states = NULL,
  ...
  if (length(states) != n_states) {
    stop(paste0(
      "The length of 'states' must equal the square root of the ",
...
tpmatrix_id <- function(object, n_samples) {
  ...
  # Create data.table
  id_names <- c(
    "strategy_id", "patient_id", "grp_id", "patient_wt",
...
#' )
#' qmatrix(fit,
#'   newdata = data.frame(age = 55, strategy_name = "New 1"),
#'   uncertainty = "none"
#' )
#' qmatrix(fit,
#'   newdata = data.frame(age = 55, strategy_name = "New 1"),
