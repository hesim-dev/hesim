...
summary.eval_rng <- function(object, probs = c(.025, .975), sep = "_", ...) {
  res_list <- lapply(seq_along(object), function(i, x, names) {
    ...
    summarize_params(x[[i]],
      param_name = "param", param_values = param_values,
      probs = probs
    )
...
check.eval_rng <- function(object) {
  ...
  fun <- function(z) {
    if (length(dim(z)) == 2) {
      ...
      stop(paste0(
        "Each element returned by define_rng() must either be a ",
...
define_model <- function(tparams_def, rng_def, params = NULL,
  ...
  x <- list(
    tparams_def = tparams_def, rng_def = rng_def, params = params,
    n_states = n_states
  )
  class(x) <- "model_def"
  check(x)
...
check.model_def <- function(x) {
  if (!all(sapply(x$tparams_def, function(z) inherits(z, "tparams_def")))) {
    stop(paste0(
      "tparams_def must either be of class 'tparams_def'",
...
eval_model <- function(x, input_data) {
  ...
  data <- data.table(
    sample = rep(1:n_samples, each = n_obs),
    data
  )
  ...
  check_n_states <- function(z, name) {
    if (length(dim(z)) == 2) {
      if (ncol(z) != (x$n_states - 1)) {
        stop(paste0(
          "The number of columns in ", name, " must equal ",
          "'n_states' - 1."
        ),
        call. = FALSE
        )
