...
hesim_data <- function(strategies, patients, states = NULL,
                       transitions = NULL) {
  object <- new_hesim_data(
    strategies = strategies, patients = patients,
...
expand.hesim_data <- function(object, by = c("strategies", "patients"),
  ...
  dat <- data.table(Reduce(
    function(...) merge(..., by = NULL, sort = FALSE),
    tbl_list
  ))
...
time_intervals <- function(times) {
  if (inherits(times, "data.frame")) {
    if (!"time_start" %in% colnames(times)) {
      stop(paste0(
        "If 'time_start' is a data frame, then 'times' ",
    ...
    if (!any(times[["time_start"]] <= 0)) {
      stop(paste0(
        "If 'times' is a data.frame, then the column ",
...
check.id_attributes <- function(object) {
  ...
  id_vars <- c(
    "sample", "strategy_id", "patient_id", "state_id",
    "transition_id", "time_id"
  )
  id_vars_n <- c(
    "n_samples", "n_strategies", "n_patients", "n_states",
  ...
  if (sum(actual_N != expected_N) > 0) {
    stop(paste0(
      "The length of the ID variables is not consistent with the number ",
      "of unique values of each ID variable."
    ), call. = FALSE)
  }
  ...
  if (!all(indices_df$row_num == sorted_seq)) {
    msg <- paste0(
      "The ID variables are not sorted correctly. The sort priority of the ",
  ...
  for (i in 2:ncol(indices_df)) {
    ...
    if (!all(unique(len) == user_n)) {
      msg <- paste0(
        "The number of unique ", col, " observations within each ",
