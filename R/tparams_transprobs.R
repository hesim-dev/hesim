...
tparams_transprobs_id <- function(x) {
  ...
  if (!is.null(x[["time_start"]])) {
    ...
    id_args[["time_intervals"]] <- data.table(
      time_id = 1,
      time_start = 0,
      time_stop = Inf
    )
...
new_tparams_transprobs_array6 <- function(object, times = NULL,
  ...
  if (!is.null(grp_id) | !is.null(patient_wt)) {
    ...
    id_df <- cbind(
      id_df,
      tmp_dt[
        match(id_df$patient_id, tmp_dt$patient_id),
        list(grp_id, patient_wt)
      ]
    )
  }
  n_df <- list(
    n_samples = dim(object)[1],
  ...
  if (n_df$n_times == 1) {
    ...
    if (is.null(times)) {
      stop(paste0(
        "'times' cannot be NULL if the number of time ",
        "intervals is greater than 1"
      ), call. = FALSE)
    }
...
new_tparams_transprobs.array <- function(object, tpmatrix_id = NULL, times = NULL,
  ...
  if (n_dim == 6) {
    ...
    if (nrow(tpmatrix_id) != dim(object)[3]) {
      stop(paste0(
        "The third dimension of the array 'object' must equal the ",
        "number or rows in 'tpmatrix_id'."
      ), call. = FALSE)
    }
...
tparams_transprobs.array <- function(object, tpmatrix_id = NULL, times = NULL,
                                     grp_id = NULL, patient_wt = NULL, ...) {
  res <- new_tparams_transprobs.array(
    object = object, tpmatrix_id = tpmatrix_id,
...
summary.tparams_transprobs <- function(object, probs = NULL,
  ...
  summarize_transprobs_dt(object_dt,
    probs = probs, unflatten = unflatten,
