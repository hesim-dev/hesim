...
check.input_mats <- function(object) {
  ...
  id_vars <- c(
    "strategy_id", "patient_id", "state_id", "transition_id",
    "time_id"
  )
  id_vars_n <- c(
    "n_strategies", "n_patients", "n_states", "n_transitions",
    "n_times"
  )
  for (i in 1:length(id_vars)) {
    if (!is.null(object[[id_vars[i]]])) {
      if (length(object[[id_vars[i]]]) != X_nrows[1]) {
        msg <- paste0(
          "The length of '", id_vars[i], "' does not equal the number of rows in the ",
          "'X' matrices."
        )
        stop(msg, call. = FALSE)
      }
...
create_input_mats_flexsurvreg_X <- function(object, input_data, ...) {
  ...
  if (length(missing.covs) > 0) {
    ...
    stop(
      sprintf("Value%s of covariate%s ", plural, plural),
...
create_input_mats_multinom_X <- function(object, input_data, ...) {
  ...
  m <- stats::model.frame(terms, input_data,
